...
describe("RecommendationRequestsIndexPage tests", () => {
  ...
  test("renders three RecommendationRequests correctly for regular user", async () => {
    ...
    const requesterEmail = screen.getByText("bob_gaucho@ucsb.edu");
    expect(requesterEmail).toBeInTheDocument();

    const professorEmails = screen.getAllByText("pconrad@ucsb.edu");
    expect(professorEmails.length).toBeGreaterThan(0);

    const explanation = screen.getByText(
      "recommendation request for the team02 project",
    );
    expect(explanation).toBeInTheDocument();

    const dateRequested = screen.getByText("2025-11-01T12:23");
    expect(dateRequested).toBeInTheDocument();

    const dateNeeded = screen.getByText("2026-02-01T12:00");
